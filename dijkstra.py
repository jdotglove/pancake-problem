...
def flipFunc(parent, index):
    ...
    # Notice in this implementation since the priority is only g(n)
    # meaning the only thing we consider is the path to that state
    # and no forward cost of how many pancakes are still out of order
    node.cost = parent.cost + (index + 1)
